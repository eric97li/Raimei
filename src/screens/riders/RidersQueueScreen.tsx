...
const RidersQueueScreen = () => {
  ...
  const [rideRequests, updateRideRequests] = useState<any[]>([]);
  const [reserveRequests, updateReserveRequests] = useState<any[]>([]);
      ...
      (async () => {
        ...
        axios(config)
            .then(function (response) {
                ...
                //filter the excess ride request objects
                const excessRideRequest = response.data.documents;
                const filteredRideRequests = [];
                for (const rideRequest of excessRideRequest) {
                  if(rideRequest.username !== username) {
                    filteredRideRequests.push(rideRequest);
                  }
                }

                //ride request objects
                updateRideRequests(filteredRideRequests);
        ...
        (async () => {
          ...
          axios(config)
              .then(function (response) {
                  ...
                  //filter the excess reserve ride objects
                  const excessReserveRequest = response.data.documents;
                  const filteredReserveRequests = [];
                  for (const reserveRequest of excessReserveRequest) {
                    if(reserveRequest.username !== username) {
                      filteredReserveRequests.push(reserveRequest);
                    }
                  }

                  //reserve ride objects
                  updateReserveRequests(filteredReserveRequests);
                    ...
                    <CollapseHeader>
                      ...
                      <CollapseBody>
                        <ListItem key={rideRequest._id} bottomDivider>
                          <ListItem.Content>
                            <ListItem.Subtitle>
                            <View><Text>Rider Details {'->'} </Text></View>
                            <View><Text> Rider Rating: {rideRequest.riderRating} </Text></View>
                            <View><Text>, Ride Count: {rideRequest.rideCount} </Text></View>
                      ...
                      <CollapseBody>
                        <ListItem key={reserveRequest._id} bottomDivider>
                          <ListItem.Content>
                            <ListItem.Subtitle>
                            <View><Text>Rider Details {'->'} </Text></View>
                            <View><Text> Rider Rating: {reserveRequest.riderRating} </Text></View>
                            <View><Text>, Ride Count: {reserveRequest.rideCount} </Text></View>
